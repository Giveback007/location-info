<script lang="ts">
    ...
    let geoLocs: Promise<WikiLoc[]> | undefined = $state();
    ...
    async function searchGeo(search: str): Promise<WikiLoc[]> {
        // await wait(1500)
...
<div>
    ...
    {:then locs}
        <!-- {#each locs as loc} -->
            <SearchList data={locs} isOpen={true} />
            <!-- <SearchCard
                title={loc.title}
                description={loc.description || loc.excerpt}
                img={loc.thumbnail?.url}
            /> -->
        <!-- {/each} -->
    {:catch e}
